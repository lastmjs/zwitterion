module.exports = {
    visitor: {
        'ImportDeclaration': (path) => {
            ...
            const hasMJSExtension = path.node.source.value.length - path.node.source.value.lastIndexOf('.mjs') === 4;
            ...
            path.node.source.value = hasExtraTSExtension ? path.node.source.value.replace('.ts', '') : hasExtraJSExtension ? path.node.source.value.replace('.js', '') : isRelativePath && (hasJSExtension || hasMJSExtension) ? path.node.source.value : isRelativePath && !hasTSExtension && !hasJSXExtension && !hasTSXExtension ? `${path.node.source.value}.ts` : path.node.source.value;
        }
    }
};
