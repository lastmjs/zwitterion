module.exports = {
    visitor: {
        'ImportDeclaration': (path) => {
            ...
            path.node.source.value = hasExtraTSExtension ? path.node.source.value.replace('.ts', '') : hasExtraJSExtension ? path.node.source.value.replace('.js', '') : isRelativePath && hasJSExtension ? path.node.source.value : isRelativePath && !hasTSExtension ? `${path.node.source.value}.ts` : path.node.source.value;
        }
    }
};
